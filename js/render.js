...
function renderBattle() {
  ...
  party.forEach((m,i)=>{
    ...
    drawHPBar('HP', m.hp, m.maxHp, x, 376, 110);
    drawMPBar(m.mp, m.maxMp, x, 390, 110);
    drawText(`${m.mp}/${m.maxMp}MP`, x, 396, '#6699ff', 9);
    if (m.isGuarding) drawText('🛡ぼうぎょ中', x, 408, '#aaffaa', 9);
  });
  ...
  if (bd.phase==='victory') {
    ...
    drawBox(180,150,280,160,'しょうり！');
    drawText(`経験値 +${expSum} EXP`, 200, 180, '#ffdd44', 16);
    drawText(`お金  +${gldSum} G`, 200, 205, '#ffcc22', 16);
    drawText('続ける...', 200, 240, '#aabbff', 14);
  }
  if (bd.phase==='defeat') {
...
function renderMenu() {
  ...
  ctx.fillStyle='rgba(0,0,0,0.5)'; ctx.fillRect(0,0,640,480);
  if (!m.subMode) {
    drawBox(440,80,180,120,'メニュー');
    m.items.forEach((item,i)=>{ const sel=i===m.selected; drawText((sel?'▶':' ')+item,455,100+i*28, sel?'#fff':'#aabbff',15); });
    drawBox(440,210,180,40); drawText(`💰 所持金: ${GameState.gold}G`,455,222,'#ffdd44',14);
